...
def register_size_routes(router: APIRouter, size: TranslationSize):
    ...
    async def list_translations(
        _: CurrentUser, session: SessionDep, pagination: PaginationSchemaDep
    ):
        rows = await get_translations(
            size, session, page=pagination.page, count=pagination.count
        )
        return rows
    ...
    @router.post(
        ...
        response_model=TranslationCreateResponseSchema,
    )
    ...
    async def get_one(
        _: CurrentUser, key: TranslationKey, lang: LanguageCode, session: SessionDep
    ):
