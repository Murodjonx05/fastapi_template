...
async def auth_user(request: Request, user: UserAuthSchema):
    try:
        user_id = await authenticate_user(user)
...
async def create_user_endpoint(request: Request, user: UserCreateSchema):
    try:
